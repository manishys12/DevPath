...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            flash('An account with that email already exists. Please use a different email.', 'danger')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            flash('Login Unsuccessful. No account found with that email.', 'danger')
            return render_template('auth/login.html', title='Login', form=form)

        logger.debug(f"User found: {user.username}, Password hash: {user.password[:10]}...")

        # Try both password verification methods
        bcrypt_check = bcrypt.check_password_hash(user.password, form.password.data)
        model_check = user.check_password(form.password.data)

        logger.debug(f"Password verification results - bcrypt direct: {bcrypt_check}, model method: {model_check}")

        # Check if password is valid using either method
        if not (bcrypt_check or model_check):
